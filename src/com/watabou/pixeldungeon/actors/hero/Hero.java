...
public class Hero extends Char {
			...
			if (heap != null) {				
				...
				if (item.doPickUp( this )) {
					...
					if (item instanceof Dewdrop) {
						...
						if (important) {
							GLog.p( HeroData.Common.TXT_YOU_NOW_HAVE, item.name() );
						} else {
							GLog.i( HeroData.Common.TXT_YOU_NOW_HAVE, item.name() );
						}
					}
					
					if (!heap.isEmpty()) {
						GLog.i( HeroData.Common.TXT_SOMETHING_ELSE );
					}
					curAction = null;
				} else {
		...
		if (Level.adjacent( pos, dst ) || pos == dst) {
			...
			if (heap != null && (heap.type != Type.HEAP && heap.type != Type.FOR_SALE)) {
				...
				if (heap.type == Type.LOCKED_CHEST || heap.type == Type.CRYSTAL_CHEST) {
					...
					if (theKey == null) {
						GLog.w( HeroData.Common.TXT_LOCKED_CHEST );
						ready();
						return false;
					}
				}
	...
	private boolean actUnlock( HeroAction.Unlock action ) {
		...
		if (Level.adjacent( pos, doorCell )) {
			...
			if (theKey != null) {
				...
				GLog.w( HeroData.Common.TXT_LOCKED_DOOR );
				ready();
			}
		...
		if (pos == stairs && pos == Dungeon.level.entrance) {
			...
			if (Dungeon.depth == 1) {
				...
				if (belongings.getItem( Amulet.class ) == null) {
					GameScene.show( new WndMessage(HeroData.Common.TXT_LEAVE) );
					ready();
				} else {
					Dungeon.win( HeroData.ResultDescriptions.RESULT_WIN);
	...
	public void earnExp( int exp ) {
		...
		if (levelUp) {
			...
			GLog.p(HeroData.Common.TXT_NEW_LEVEL, lvl );
			sprite.showStatus( CharSprite.POSITIVE, HeroData.Common.TXT_LEVEL_UP);
	...
	public void add( Buff buff ) {
		...
		if (sprite != null) {
			if (buff instanceof Burning) {
				GLog.w(HeroData.Buff.HERO_TXT_FIRE);
				interrupt();
			} else if (buff instanceof Paralysis) {
				GLog.w(HeroData.Buff.HERO_TXT_PARALYSED);
				interrupt();
			} else if (buff instanceof Poison) {
				GLog.w(HeroData.Buff.HERO_TXT_POISONED);
				interrupt();
			} else if (buff instanceof Ooze) {
				GLog.w(HeroData.Buff.HERO_TXT_OOZE);
			} else if (buff instanceof Roots) {
				GLog.w(HeroData.Buff.HERO_TXT_ROOTED);
			} else if (buff instanceof Weakness) {
				GLog.w(HeroData.Buff.HERO_TXT_WEAKNESS);
			} else if (buff instanceof Blindness) {
				GLog.w(HeroData.Buff.HERO_TXT_BLINDED);
			} else if (buff instanceof Fury) {
				GLog.w(HeroData.Buff.HERO_TXT_FURIOUS);
				sprite.showStatus( CharSprite.POSITIVE, HeroData.Sprite.SPRITE_FURIOUS);
			} else if (buff instanceof Charm) {
				GLog.w(HeroData.Buff.HERO_TXT_CHARM);
			}  else if (buff instanceof Cripple) {
				GLog.w(HeroData.Buff.HERO_TXT_CRIPPLED);
			} else if (buff instanceof Bleeding) {
				GLog.w(HeroData.Buff.HERO_TXT_BLEEDING);
			} else if (buff instanceof Vertigo) {
				GLog.w(HeroData.Buff.HERO_TXT_DIZZY);
				interrupt();
			}
	...
	public boolean search( boolean intentional ) {
		...
		if (intentional) {
			sprite.showStatus( CharSprite.DEFAULT, HeroData.Common.TXT_SEARCH );
			sprite.operate( pos );
			if (smthFound) {
		...
		if (smthFound) {
			GLog.w( HeroData.Common.TXT_NOTICED_SMTH );
