...
public class Hunger extends Buff implements Hero.Doom {
		...
		if (target.isAlive()) {
			...
			if (isStarving()) {
				if (Random.Float() < 0.3f && (target.HP > 1 || !target.paralysed)) {
					...
					GLog.n(BuffData.BUFF_HUNGER_TXT_STARVING);
					hero.damage( 1, this );
				...
				if (newLevel >= STARVING) {
					...
					GLog.w(BuffData.BUFF_HUNGER_TXT_HUNGRY);
					statusUpdated = true;
	...
	public String toString() {
		if (level < STARVING) {
			return BuffData.BUFF_HUNGER_HUNGRY;
		} else {
			return BuffData.BUFF_HUNGER_STARVING;
		}
	}
	...
	public void onDeath() {
		...
		Dungeon.fail( Utils.format( HeroData.ResultDescriptions.RESULT_HUNGER, Dungeon.depth ) );
		GLog.n(HeroData.Death.DEATH_HUNGER);
	}
}
